...
namespace CodeAutomation.Pages
{
    class Deals : IPages
    {
        public void NavigateSubMenu(IWebDriver driver, string subMenuNav)
        {
            ...
            //Hover Over
            var navBarButton = wait.Until(ExpectedConditions.ElementIsVisible(By.XPath("//*[@id=\"top_menu\"]/ul/li[4]")));
            var action = new Actions(driver);
            action.MoveToElement(navBarButton).Perform();

            IWebElement subMenuDiv = driver.FindElement(By.CssSelector("#top_menu > ul > li:nth-child(4) > ul"));
